...
struct aiColor4D getDiffuseColor(struct aiMaterial** array, unsigned int index) {
	struct aiColor4D mtlDiffuse;
	int success = aiGetMaterialColor(array[index], AI_MATKEY_COLOR_DIFFUSE, &mtlDiffuse);
...
struct aiColor4D getAmbientColor(struct aiMaterial** array, unsigned int index) {
	struct aiColor4D mtlAmbient;
	int success = aiGetMaterialColor(array[index], AI_MATKEY_COLOR_AMBIENT, &mtlAmbient);
	if (success == aiReturn_SUCCESS) {
		return mtlAmbient;
	} else {
...
struct aiColor4D getSpecularColor(struct aiMaterial** array, unsigned int index) {
	struct aiColor4D mtlSpecular;
	int success = aiGetMaterialColor(array[index], AI_MATKEY_COLOR_SPECULAR, &mtlSpecular);
	if (success == aiReturn_SUCCESS) {
		return mtlSpecular;
	} else {
...
struct aiString getDiffuseTextureMap(struct aiMaterial** array, unsigned int index) {
	int textureCount = aiGetMaterialTextureCount(array[index], aiTextureType_DIFFUSE);

	if (textureCount > 0) {
		struct aiString path;
		int success = aiGetMaterialTexture(array[index], aiTextureType_DIFFUSE, 0, &path, NULL, NULL, NULL, NULL, NULL, NULL);
		...
		//we will return some kind of an error or something
		printf("No diffuse texture for this material!\n");
		//create an aiString with a null value at the beginning to show an error
		struct aiString errorPath;
		errorPath.data[0] = -1;
		return errorPath;
	}
...
float getShininess(struct aiMaterial** array, unsigned int index) {
	float shininess[10];
	int max;
	int success = aiGetMaterialFloatArray(array[index], AI_MATKEY_SHININESS, shininess, &max);
	if (success == aiReturn_SUCCESS) {
		return shininess[0];
	} else {
		fprintf(stderr, "Error reading shininess!\n");
...
struct aiMaterial getMaterialAtIndex(struct aiMaterial** array, unsigned int index) {
	struct aiColor4D mtlDiffuse;
	int success = aiGetMaterialColor(array[index], AI_MATKEY_COLOR_DIFFUSE, &mtlDiffuse);

	if (success == aiReturn_SUCCESS) {
		printf("SUCCESS r: %f g: %f b: %f a: %f\n", mtlDiffuse.r, mtlDiffuse.g, mtlDiffuse.b, mtlDiffuse.a);
	} else {
		printf("NOT SUCCESS\n");
	}
...
func ParseFile(modelFile string) (outMeshes []MyMesh, err error) {
	///////////////////////////////////////////////////////////
	...
	for i := uint(0); i < uint(cScene.mNumMeshes); i++ {
		...
		tempMaterial := MyMaterial{}
		diffuseColor := C.getDiffuseColor(cScene.mMaterials, cMesh.mMaterialIndex)
		tempMaterial.Diffuse = []float32{float32(diffuseColor.r), float32(diffuseColor.g), float32(diffuseColor.b), float32(diffuseColor.a)}
		ambientColor := C.getAmbientColor(cScene.mMaterials, cMesh.mMaterialIndex)
		tempMaterial.Ambient = []float32{float32(ambientColor.r), float32(ambientColor.g), float32(ambientColor.b), float32(ambientColor.a)}
		specularColor := C.getSpecularColor(cScene.mMaterials, cMesh.mMaterialIndex)
		...
		//error checking if there was a diffuse map or not
		if path.data[0] != -1 {
			for j := 0; j < int(path.length); j++ {
				temp := fmt.Sprintf("%c", path.data[j])
				pathString += temp
			}

			tempMaterial.DiffuseMap = pathString
		} else {
			tempMaterial.DiffuseMap = ""
		}

		tempMyMesh.Material = tempMaterial
